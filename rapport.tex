...
\begin{consigne}[Consigne]
Précisez en deux mots le rôle du membre (ex: \emph{\og développeur du moteur physique\fg{}}, \emph{\og développeur du gameplay\fg{}}).
Chaque membre doit avoir contribué au code Java.
\end{consigne}
...
\begin{consigne}[Exemple d'objectifs]
Développer un moteur de jeu 2D extensible permettant d'ajouter du contenu sans modifier le code Java,
ou implémenter un moteur de carte 2D pour un RPG gérant les positions via Tiled.
\end{consigne}
...
% Option (facultatif) : exemple de listing
\begin{lstlisting}
// Selection d'un chemin via la Strategy pour la vague courante
...
% Exemple de structure "pas à pas" (optionnel)
\begin{consigne}[Astuce de rédaction]
Décris un \textbf{exemple concret} :
\begin{itemize}
  \item un fichier \texttt{maps/BeginningFields.tmx} ajouté,
  \item une couche de tiles pour le décor,
  \item une couche d'objets \texttt{Collisions} avec rectangles/polylines,
  \item une couche d'objets décoratifs (objets avec \texttt{gid}),
  \item et comment le moteur transforme ces objets en rectangles de collision utilisés par \texttt{PlayerTower}.
\end{itemize}
\end{consigne}

\subsection{Compilation et exécution}
\begin{consigne}
Présentez comment \textbf{compiler et exécuter} votre projet.
\end{consigne}
...
\begin{itemize}
  ...
  \item \textbf{UI / Contrôleurs} : \texttt{UIManager}, \texttt{GameInputController}, \texttt{PlacementController},
        \texttt{GameController}.
  \item \textbf{Modèle central} : \texttt{GameState} regroupe l'état partagé (joueur, vagues, entités, économie).
...
' Notes
note right of ProjectilePool
  Singleton pool that creates Projectile or
  RicochetProjectile based on ProjectileType.
end note

note left of ProjectileTypeFactory
  Flyweight factory for projectile intrinsic data.
end note
...
\begin{figure}[H]
  \centering
  \resizebox{0.5\linewidth}{!}{\UMLInput{pattern_factory}}
\end{figure}
...
\begin{figure}[H]
  \centering
  \resizebox{0.85\linewidth}{!}{\UMLInput{pattern_manager}}
\end{figure}
...
\subsubsection{UML - Sous-système Projectile}
La structure du sous-système Projectile s'appuie sur la factory et le flyweight.
\begin{figure}[H]
  \centering
  \resizebox{0.5\linewidth}{!}{\UMLInput{pattern_factory}}
\end{figure}
...
Le déplacement est désormais piloté par le contrôleur d'entrée : il calcule une direction
continue et la transmet au modèle (joueur) et à la caméra. Cela évite les lectures directes
dans les entités et renforce la séparation MVC.
...
L'interface (HUD, menus) s'appuie sur Scene2D. Le contrôleur d'entrée encapsule un
\texttt{InputMultiplexer} pour faire cohabiter UI et gameplay.
